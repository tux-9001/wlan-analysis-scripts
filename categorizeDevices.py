...
class dataFile:
    def __init__(self, cameraCount, smallDeviceCount, evChargingCount, homeMgmtCount, securityCount, totalIoTCount, totalDeviceCount):
            ...
            for device in network.stationList:
                ...
                if isIOT(device):
                    ...
                    if category == "security":
                        securityCount += 1
        df = dataFile(cameraCount, smallDeviceCount, evChargingCount, homeMgmtCount, securityCount, totalIoTcount, totalDeviceCount)
...
print("Cams pwr ev hmmnt sec")
print(x)
df = pd.DataFrame(x,
                 index=y,
                 columns=pd.Index(['Cams', 'Pwr', 'EV.', 'HmMnt.', 'Sec.', ' '], 
                 name='Types of IoT devices')).round(2)
