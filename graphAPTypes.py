    ...
    for network in networkArray:
        ...
        if isIoT:
            ...
            for name in iotApplianceSubstrings: 
                if name in ssidU:
                    iotAppliances += 1
                    print(str(network.ssid)+" < Appliance")
            for name in iotSetupSubstrings:
                if name in ssidU: 
                    iotSetupNets += 1
                    print(str(network.ssid)+" < Setup")
            for name in iotPrivCameraSubstrings: 
                if name in ssidU:
                    iotCameraNets += 1 
                    print(str(network.ssid)+" < Private camera network")
            iotNetworkCount += 1  
            continue
        if isStandard: 
            stdNetworkCount += 1
    print("Appliances: "+str(iotAppliances)+" Setup "+str(iotSetupNets)+" Cameras "+str(iotCameraNets))
    labels =  'Printers', 'IoT', 'Direct (others)' 
    sizes = [ printerNetworkCount, iotNetworkCount, directNetworkCount]
    sumSizes = 0 
    for size in sizes:
        sumSizes += size
        print(str(size))
